...
@section('toolbar')
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-4 w-100">
        <div>
            ...
            <div class="text-muted fw-semibold fs-7 pt-1">Administra WordPress, Facebook, Instagram y X desde un solo lugar.</div>
        </div>
...
@section('content')
    @if ($sites->isEmpty())
        <div class="card card-flush"><div class="card-body text-center py-15">
            ...
            <p class="text-muted fw-semibold mb-7">Después podrás publicar cada post generado en WordPress, Facebook, Instagram o X.</p>
        ...
        <div class="row g-7">
            @foreach ($sites as $site)
                @php
                    ...
                    $accountLabel = match ($site->type) {
                        App\Models\WordPressSite::TYPE_FACEBOOK_PAGE => ['Página', $site->facebook_page_id],
                        App\Models\WordPressSite::TYPE_INSTAGRAM => ['Cuenta', $site->instagram_account_id],
                        App\Models\WordPressSite::TYPE_X => ['Usuario', $site->x_username ? '@'.$site->x_username : '—'],
                        default => ['Usuario', $site->username],
                    };
                @endphp
                <div class="col-xl-6">
                    <div class="card card-flush h-100">
                        <div class="card-body d-flex flex-column gap-5">
                            ...
                            <div class="d-flex flex-wrap gap-8 text-gray-700">
                                <div>
                                    <span class="text-muted fs-8 d-block">{{ $accountLabel[0] }}</span>
                                    <span class="fw-bold">{{ $accountLabel[1] }}</span>
                                </div>
