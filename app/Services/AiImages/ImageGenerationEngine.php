...
class ImageGenerationEngine
{
    ...
    public function prepare(string $type, array $context, array $options = []): ImageGenerationResult
    {
        ...
        $outputFormat = (string) ($options['output_format'] ?? 'jpeg');
        $outputCompression = (int) ($options['output_compression'] ?? 85);
        ...
        if ($outputFormat !== 'png') {
            $requestOptions['output_compression'] = $outputCompression;
        }

        $request = $this->openAI->images->generate($prompt, $requestOptions);
    ...
    public function complete(AiImage $image, array|string $response, ?string $imageUrl = null, array $metrics = []): AiImage
    {
        $image->update([
            ...
            'output_format' => $metrics['output_format'] ?? $image->output_format,
            'output_compression' => $metrics['output_compression'] ?? $image->output_compression,
